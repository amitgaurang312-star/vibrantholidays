...
export default function TestimonialsSection() {
  ...
  return (
    <section ref={sectionRef} className="relative py-14 sm:py-20 lg:py-28 overflow-hidden" style={{ background: 'linear-gradient(180deg, #F7F9FC 0%, #FFFFFF 100%)' }}>
      {/* Decorative orbs */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          {testimonials?.map((t, i) => (
            <div
              key={t?.id}
              className={`reveal-scale ${visible ? 'active' : ''} group relative bg-white rounded-2xl p-5 sm:p-6 border border-border/60 card-hover overflow-hidden`}
              style={{
            ...
            >
              ...
              <div className="absolute top-4 right-4 opacity-6 group-hover:opacity-10 transition-opacity duration-300">
                <svg className="w-10 sm:w-12 h-10 sm:h-12" style={{ color: t?.color }} fill="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex gap-0.5 mb-3 sm:mb-4">
                {[...Array(t?.rating)]?.map((_, si) => (
                  <svg key={si} className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-accent" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                  </svg>
                ))}
              </div>

              <p className="text-foreground/80 text-sm leading-relaxed mb-4 sm:mb-5 line-clamp-4">
                &ldquo;{t?.text}&rdquo;
              </p>

              {/* Author */}
              <div className="flex items-center gap-3 pt-3 sm:pt-4 border-t border-border/60">
                <div
                  className="w-9 sm:w-10 h-9 sm:h-10 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0"
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-1 mt-0.5">
                    <GoogleIcon />
                    <p className="text-muted-foreground text-xs truncate">{t?.location}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Google Rating Badge */}
        <div className={`mt-10 sm:mt-14 flex justify-center reveal-up ${visible ? 'active' : ''}`} style={{ transitionDelay: '500ms' }}>
          <div
            className="flex items-center gap-3 sm:gap-4 px-5 sm:px-7 py-3 sm:py-4 rounded-2xl"
            style={{
              background: 'white',
          ...
          >
            ...
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                <span className="text-foreground font-bold text-sm sm:text-base">4.9</span>
                <div className="flex gap-0.5">
                  {[...Array(5)]?.map((_, i) => (
                    <svg key={i} className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-accent" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                    </svg>
                  ))}
                </div>
