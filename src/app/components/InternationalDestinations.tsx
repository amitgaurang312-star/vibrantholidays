  ...
  return (
    <div
      ...
      style={{
        ...
        minHeight: dest.name === 'Maldives' ? 220 : 260,
      ...
      <div className="absolute inset-0 flex flex-col justify-between p-4 sm:p-6 z-10">
        <div className="flex justify-between items-start gap-2">
          <span className="glass-panel text-white text-xs font-medium px-2.5 sm:px-3 py-1.5 rounded-full flex items-center gap-1.5">
            <svg className="w-3 h-3 text-white/70 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            {dest.duration}
          </span>
          <span className="text-white text-xs font-bold px-2.5 sm:px-3 py-1.5 rounded-full flex-shrink-0" style={{ background: 'linear-gradient(135deg, #C8860E, #F0BC4A)', boxShadow: '0 2px 8px rgba(216,154,36,0.5)' }}>
            From {dest.price}
          </span>
        </div>

        <div>
          <p className="text-white text-xs font-semibold uppercase tracking-widest mb-1" style={{ textShadow: '0 1px 4px rgba(0,0,0,0.8)' }}>{dest.tagline}</p>
          <h3 className="font-display text-xl sm:text-card-lg text-white font-semibold mb-2 sm:mb-3" style={{ textShadow: '0 2px 8px rgba(0,0,0,0.9)' }}>{dest.name}</h3>
          <div className="flex flex-wrap gap-1 sm:gap-1.5 mb-3 sm:mb-4">
            {dest.highlights?.map((h) =>
            <span key={h} className="text-white text-xs px-2 sm:px-2.5 py-1 rounded-full font-medium" style={{ background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.2)' }}>
                {h}
              </span>
            )}
          </div>
          <Link
            ...
            className="inline-flex items-center gap-2 bg-white text-primary px-4 sm:px-5 py-2.5 rounded-full text-sm font-semibold sm:opacity-0 sm:group-hover:opacity-100 sm:translate-y-3 sm:group-hover:translate-y-0 touch-manipulation"
  ...
  return (
    <section ref={sectionRef} className="relative py-14 sm:py-20 lg:py-28 overflow-hidden" style={{ background: 'linear-gradient(180deg, #EEF3F8 0%, #F7F9FC 100%)' }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
        {/* Header */}
        <div className={`flex flex-col sm:flex-row sm:items-end justify-between gap-3 sm:gap-4 mb-8 sm:mb-12 reveal-up ${visible ? 'active' : ''}`}>
          <div>
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-widest mb-4 sm:mb-5" style={{ background: 'rgba(216,154,36,0.10)', color: '#D89A24', border: '1px solid rgba(216,154,36,0.20)' }}>
              <span className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse" />
              International Destinations
            </span>
            <h2 className="font-display text-3xl sm:text-section-xl text-foreground font-semibold">
              Explore the <span className="text-gradient-gold">World</span>
            </h2>
          </div>
          <Link
            href="/tour-packages"
            className="flex items-center gap-2 text-primary font-semibold text-sm hover:gap-3 transition-all duration-300 flex-shrink-0 group touch-manipulation self-start sm:self-auto">
        ...
        {/* CTA Banner */}
        <div
          className={`mt-8 sm:mt-12 rounded-3xl p-6 sm:p-8 lg:p-12 flex flex-col lg:flex-row items-center justify-between gap-5 sm:gap-6 reveal-up ${visible ? 'active' : ''} relative overflow-hidden`}
          style={{
          ...
          <div className="relative z-10 text-center lg:text-left">
            <h3 className="font-display text-xl sm:text-2xl lg:text-3xl font-semibold text-white mb-2">
          ...
          <Link
            href="/contact"
            className="relative z-10 flex-shrink-0 flex items-center gap-2.5 bg-white text-primary px-6 sm:px-8 py-3.5 sm:py-4 rounded-full font-semibold text-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:scale-105 touch-manipulation w-full sm:w-auto justify-center"
