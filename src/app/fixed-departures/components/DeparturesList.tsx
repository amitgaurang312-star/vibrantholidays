...
const ladakhDepartureDates: DepartureDate[] = [
...
const ladakhItinerary = [
  {
    ...
    points: [
      'Arrive at Leh Airport; transfer to hotel for check-in.',
      'Full day free for leisure & acclimatization.',
      'Overnight stay at hotel in Leh.',
    ],
  },
  {
    day: 'Day 2',
    title: 'Leh – Sangam – Hall of Fame – Leh',
    points: [
      'After breakfast, half-day excursion to Spituk Monastery.',
      'Visit Hall of Fame — a museum dedicated to Indian Army soldiers.',
      'Stop at Gurudwara Pathar Sahib, a sacred Sikh shrine.',
      'Experience the famous Magnetic Hill.',
      'Witness Sangam View — confluence of the Indus & Zanskar Rivers.',
      'Explore Leh Market and the historic Leh Palace.',
      'Visit Shanti Stupa for panoramic views of Leh valley.',
      'Afternoon drive back to hotel. Night stay in Leh.',
    ],
  },
  {
    day: 'Day 3',
    ...
    points: [
      'Early breakfast, then drive to Nubra Valley via Khardung La Pass.',
      'Khardung La at 18,380 ft — the highest motorable road in the world.',
      'Nubra Valley is popularly known as Ldorma or the valley of flowers, situated between the Karakoram and Ladakh ranges.',
      'En route visit Diskit Monastery perched on a hilltop.',
      'Arrive at Hunder; check into hotel/camp.',
      'Visit Diskit Village & enjoy a Camel Ride on the Sand Dunes.',
  ...
  },
  {
    ...
    points: [
      ...
      'Turtuk was taken over by the Indian Army during the Kargil War.',
      'Be mesmerized by the beauty and simplicity of Turtuk and its people.',
  ...
  },
  {
    ...
    points: [
      'After breakfast, drive to Pangong Lake via Shyok, Durbuk and Tangste villages.',
      'Pangong Lake lies in the Changthang region, east of Leh — one of the most amazing lakes in Asia.',
      'The lake changes its colour 4–5 times a day.',
  ...
  },
  {
    ...
    points: [
      ...
      'Visit the historic Hanle Monastery.',
      'Explore the vast Changthang Plateau.',
      'Night stay in Hanle.',
    ],
  },
  {
    ...
    points: [
      ...
      'Stop at the majestic Shey Palace.',
      'Visit Thiksey Monastery.',
      'See the famous Rancho School (Druk Padma Karpo School).',
...
const ladakhExclusions = [
  ...
  'Personal expenses (tips, laundry, telephone calls, fax, etc.)',
  'Costs incidental to any change in the itinerary',
  'Expenses due to events beyond control (bad weather, natural calamities, flight delays/rescheduling/cancellations, accidents/medical evacuations, riots/strikes/war, etc.)',
...
const thailandDepartureDates: DepartureDate[] = [
...
interface ItineraryItem { day: string; title: string; points: string[] }
interface IncExcProps { inclusions: string[]; exclusions: string[] }

function ItinerarySection({ itinerary, visible }: { itinerary: ItineraryItem[]; visible: boolean }) {
  return (
...
function InclusionsExclusions({ inclusions, exclusions, visible }: IncExcProps & { visible: boolean }) {
  return (
    <div className={`reveal-up ${visible ? 'active' : ''} mb-16`}>
...
function DepartureDatesSection({ departureDates, visible }: { departureDates: DepartureDate[]; visible: boolean }) {
        ...
        <div key={month} className="mb-10">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {departureDates
              ...
              .map((dep) => (
                <div key={dep.date} className="flex items-center gap-2 px-5 py-3 rounded-2xl bg-white border border-border/60 hover:border-primary/40 transition-all duration-200 hover:shadow-md" style={{ boxShadow: '0 2px 12px rgba(13,27,42,0.06)' }}>
...
function DestinationDivider({ visible }: { visible: boolean }) {
  return (
    <div className={`reveal-up ${visible ? 'active' : ''} my-20`}>
      <div className="flex items-center gap-6">
        <div className="h-px flex-1" style={{ background: 'linear-gradient(90deg, transparent, rgba(11,138,143,0.30))' }} />
        <div className="flex items-center gap-3 px-6 py-3 rounded-2xl" style={{ background: 'rgba(11,138,143,0.06)', border: '1px solid rgba(11,138,143,0.15)' }}>
          <span className="text-2xl">✈️</span>
          <span className="font-display text-sm font-semibold text-primary uppercase tracking-widest">More Destinations</span>
          <span className="text-2xl">🌏</span>
        </div>
        <div className="h-px flex-1" style={{ background: 'linear-gradient(90deg, rgba(11,138,143,0.30), transparent)' }} />
      </div>
    </div>
  );
}
    ...
    <section ref={sectionRef} className="relative py-16 lg:py-24 overflow-hidden" style={{ background: 'linear-gradient(180deg, #F7F9FC 0%, #FFFFFF 100%)' }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className={`text-center mb-14 reveal-up ${visible ? 'active' : ''}`}>
          ...
          <p className="text-muted-foreground text-xs mt-2">Reg. No: UDYAM-MH-26-1125371</p>
        </div>

        {/* Ladakh Hero Package Card */}
        <div className={`reveal-up ${visible ? 'active' : ''} mb-16`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2">
              <div className="relative h-72 lg:h-auto min-h-72 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex flex-col gap-2">
                  <span className="text-white text-xs font-bold px-3 py-1.5 rounded-full" style={{ background: 'linear-gradient(135deg, #C8860E, #F0BC4A)', boxShadow: '0 2px 8px rgba(216,154,36,0.5)' }}>Fixed Departure</span>
                  <span className="text-white text-xs font-bold px-3 py-1.5 rounded-full" style={{ background: 'linear-gradient(135deg, #0B8A8F, #0ABBC2)', boxShadow: '0 2px 8px rgba(11,138,143,0.4)' }}>8 Days / 7 Nights</span>
                  <span className="text-white text-xs font-bold px-3 py-1.5 rounded-full" style={{ background: 'linear-gradient(135deg, #1a5276, #2980b9)', boxShadow: '0 2px 8px rgba(26,82,118,0.4)' }}>Group Trip</span>
                </div>
              </div>
                  ...
                  <div className="rounded-2xl p-4 mb-5" style={{ background: 'rgba(200,134,14,0.06)', border: '1px solid rgba(200,134,14,0.12)' }}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                      <div>
                        ...
                        <p className="text-muted-foreground text-xs">Hotel Sindhu Residency / Hotel Holiday Ladakh / Welkin High / The Nangsay Hotel / Similar</p>
                      </div>
                      <div>
                        <p className="font-semibold text-foreground">Nubra · 2 Nights</p>
                        <p className="text-muted-foreground text-xs">The Latsas / Ldazes Camp / Hotel Thachung / Ladakh Heaven Hills / Paradise North / Similar</p>
                      </div>
                      <div>
                        <p className="font-semibold text-foreground">Pangong · 1 Night</p>
                        <p className="text-muted-foreground text-xs">Mystic Mountain Cottage / The Golden Cottage / LakeTso House Pangong / Similar</p>
                      </div>
                      <div>
                        <p className="font-semibold text-foreground">Hanle · 1 Night</p>
                        <p className="text-muted-foreground text-xs">Sonam Homestay / Dark Sky Homestay / Gua Homestay / Similar</p>
                      </div>
                    </div>
                  </div>
                </div>
                <a href="tel:+918668355974" className="inline-flex items-center justify-center gap-2 text-white px-6 py-3 rounded-full text-sm font-semibold transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg w-full sm:w-auto" style={{ background: 'linear-gradient(135deg, #0B8A8F, #067A7F)' }}>
        ...
        {/* Ladakh Departure Dates */}
        <DepartureDatesSection departureDates={ladakhDepartureDates} visible={visible} />

        {/* Ladakh Itinerary */}
        <ItinerarySection itinerary={ladakhItinerary} visible={visible} />

        {/* Ladakh Inclusions & Exclusions */}
        <InclusionsExclusions inclusions={ladakhInclusions} exclusions={ladakhExclusions} visible={visible} />

        {/* Divider between destinations */}
        <DestinationDivider visible={visible} />
          ...
          <div className="bg-white rounded-3xl overflow-hidden border border-border/60" style={{ boxShadow: '0 8px 40px rgba(13,27,42,0.10)' }}>
            <div className="grid grid-cols-1 lg:grid-cols-2">
              <div className="relative h-72 lg:h-auto min-h-72 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex flex-col gap-2">
                  <span className="text-white text-xs font-bold px-3 py-1.5 rounded-full" style={{ background: 'linear-gradient(135deg, #C8860E, #F0BC4A)', boxShadow: '0 2px 8px rgba(216,154,36,0.5)' }}>Fixed Departure</span>
                  <span className="text-white text-xs font-bold px-3 py-1.5 rounded-full" style={{ background: 'linear-gradient(135deg, #0B8A8F, #0ABBC2)', boxShadow: '0 2px 8px rgba(11,138,143,0.4)' }}>5 Days / 4 Nights</span>
                </div>
              </div>
              <div className="p-8 flex flex-col justify-between">
                <div>
        ...
        <DepartureDatesSection departureDates={thailandDepartureDates} visible={visible} />

        {/* Thailand Itinerary */}
        <ItinerarySection itinerary={thailandItinerary} visible={visible} />

        {/* Thailand Inclusions & Exclusions */}
        <InclusionsExclusions inclusions={thailandInclusions} exclusions={thailandExclusions} visible={visible} />
        ...
        <div className={`reveal-up ${visible ? 'active' : ''}`}>
          <div className="rounded-3xl p-8 lg:p-12 text-center text-white" style={{ background: 'linear-gradient(135deg, #0B8A8F 0%, #0ABBC2 100%)', boxShadow: '0 8px 40px rgba(11,138,143,0.35)' }}>
            ...
            <h3 className="font-display text-3xl lg:text-4xl font-semibold mb-3">
              Ready to Book Your Next Adventure?
            </h3>
            <p className="text-white/80 text-lg mb-6 max-w-xl mx-auto">
              Seats fill up fast on a first-come, first-served basis. Call us now to secure your spot for Ladakh or Thailand.
            </p>
            <a href="tel:+918668355974" className="inline-flex items-center gap-2 bg-white font-semibold px-8 py-3.5 rounded-full text-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl" style={{ color: '#0B8A8F' }}>
