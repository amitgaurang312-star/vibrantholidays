...
export default function DeparturesList() {
  ...
  const [activeTab, setActiveTab] = useState<'thailand' | 'ladakh'>('thailand');
  ...
  const isThailand = activeTab === 'thailand';

  const departureDates = isThailand ? thailandDepartureDates : ladakhDepartureDates;
  const itinerary = isThailand ? thailandItinerary : ladakhItinerary;
  const inclusions = isThailand ? thailandInclusions : ladakhInclusions;
  const exclusions = isThailand ? thailandExclusions : ladakhExclusions;
  ...
  return (
    <section ref={sectionRef} className="relative py-16 lg:py-24 overflow-hidden" style={{ background: 'linear-gradient(180deg, #F7F9FC 0%, #FFFFFF 100%)' }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
        ...
        {/* Destination Tabs */}
        <div className={`flex justify-center mb-12 reveal-up ${visible ? 'active' : ''}`}>
          <div className="inline-flex rounded-2xl p-1.5 gap-1" style={{ background: 'rgba(11,138,143,0.08)', border: '1px solid rgba(11,138,143,0.15)' }}>
            <button
              onClick={() => setActiveTab('thailand')}
              className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold transition-all duration-300 ${isThailand ? 'text-white shadow-md' : 'text-muted-foreground hover:text-foreground'}`}
              style={isThailand ? { background: 'linear-gradient(135deg, #0B8A8F, #0ABBC2)', boxShadow: '0 4px 16px rgba(11,138,143,0.35)' } : {}}>
              <span>🇹🇭</span>
              Thailand
            </button>
            <button
        ...
        {/* Departure Dates by Month (shared pattern) */}
        <div className={`reveal-up ${visible ? 'active' : ''} mb-16`}>
          <h3 className="font-display text-2xl font-semibold text-foreground mb-8 text-center">
        ...
        {/* Itinerary */}
        <ItinerarySection itinerary={itinerary} visible={visible} />

        {/* Inclusions & Exclusions */}
        <InclusionsExclusions inclusions={inclusions} exclusions={exclusions} visible={visible} />
        ...
        <div className={`reveal-up ${visible ? 'active' : ''}`}>
          <div className="rounded-3xl p-8 lg:p-12 text-center text-white" style={{ background: 'linear-gradient(135deg, #0B8A8F 0%, #0ABBC2 100%)', boxShadow: '0 8px 40px rgba(11,138,143,0.35)' }}>
            ...
            <h3 className="font-display text-3xl lg:text-4xl font-semibold mb-3">
              {isThailand ? 'Ready to Explore Thailand?' : 'Ready to Explore Ladakh?'}
            </h3>
